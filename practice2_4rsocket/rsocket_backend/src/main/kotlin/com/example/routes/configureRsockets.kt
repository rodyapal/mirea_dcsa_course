		...
		RSocketRequestHandler {
			...
			requestChannel { initPayload, payloads ->
				...
				payloads.onEach { payload ->
					...
					println("Received extra payload, changed emitted values to include prefix: '$prefix'")
				}.launchIn(this)

				flow {
					emitDataContinuously(prefix, dao)
...
private suspend fun FlowCollector<Payload>.emitDataContinuously(
	...
	while (true) {
		val data = dao.article(prefix.filter { it.isDigit() }.toInt())
